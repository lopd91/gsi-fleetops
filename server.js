...
app.use(session({
  secret: process.env.SESSION_SECRET || 'gsi-change-this-secret',
  resave: false,
...
app.post('/api/auth/login', (req, res) => {
  try {
    ...
    const user = repo.get('SELECT * FROM users WHERE username=? AND active=1', [username.trim()]);
...
app.post('/api/users', admin, (req,res) => {
  ...
  if (repo.get('SELECT id FROM users WHERE username=?', [username.toLowerCase()])) return res.status(400).json({ error:'Username already exists' });
  const id = repo.uid();
...
// Generic CRUD - use inline table queries instead of makeRepo
const TABLES = {
  bookings: { cols: 'id,job,date,client,contact,origin,destination,pickup,delivery,truck_id,driver_id,cargo,weight,amount,status,notes,doc,created_by,updated_by,created_at,updated_at' },
  drivers:  { cols: 'id,name,ic,phone,license,expiry,status,cicpa,cicpa_expiry,created_by,updated_by,created_at,updated_at' },
  trucks:   { cols: 'id,plate,model,type,capacity,roadtax,status,owner,vendor_id,created_by,updated_by,created_at,updated_at' },
  vendors:  { cols: 'id,num,name,cat,contact,phone,email,trn,terms,status,notes,created_by,updated_by,created_at,updated_at' },
  clients:  { cols: 'id,num,type,name,contact,phone,email,trn,address,credit,terms,notes,created_by,updated_by,created_at,updated_at' },
  vouchers: { cols: 'id,num,date,cat,description,requested_by,amount,job_ref,status,photo,created_by,updated_by,created_at,updated_at' },
  receipts: { cols: 'id,num,date,cat,vendor,job_ref,driver_id,description,amount,photo,created_by,updated_by,created_at,updated_at' }
};

Object.keys(TABLES).forEach(name => {
  app.get('/api/'+name, auth, (req,res) => {
    res.json(repo.all('SELECT * FROM '+name+' ORDER BY created_at DESC'));
  });
  app.get('/api/'+name+'/:id', auth, (req,res) => {
    const row = repo.get('SELECT * FROM '+name+' WHERE id=?',[req.params.id]);
    if (!row) return res.status(404).json({error:'Not found'});
    res.json(row);
  });
  app.post('/api/'+name, auth, (req,res) => {
    try {
      const record = Object.assign({}, req.body, {created_by:req.session.user.id, created_at:new Date().toISOString()});
      const keys = Object.keys(record);
      const placeholders = keys.map(()=>'?').join(',');
      repo.run('INSERT INTO '+name+' ('+keys.join(',')+') VALUES ('+placeholders+')', Object.values(record));
      res.json(record);
    } catch(e) { res.status(400).json({error:e.message}); }
  });
  app.put('/api/'+name+'/:id', auth, (req,res) => {
    try {
      const record = Object.assign({}, req.body, {updated_by:req.session.user.id});
      const keys = Object.keys(record);
      const sets = keys.map(k=>k+'=?').join(',');
      repo.run('UPDATE '+name+' SET '+sets+',updated_at=datetime("now") WHERE id=?', [...Object.values(record), req.params.id]);
      res.json(repo.get('SELECT * FROM '+name+' WHERE id=?',[req.params.id]));
    } catch(e) { res.status(400).json({error:e.message}); }
  });
  app.delete('/api/'+name+'/:id', auth, (req,res) => {
    repo.run('DELETE FROM '+name+' WHERE id=?',[req.params.id]);
    res.json({success:true});
  });
});
